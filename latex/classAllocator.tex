...
\item 
void \hyperlink{classAllocator_aaf1dbab9be62941817f98618f94c7146}{deallocate} (pointer p, size\-\_\-type t)
\item 
...
O(1) in space O(1) in time Default constructor that creates and initializes \hyperlink{classAllocator}{Allocator} object for use. throw a bad\-\_\-alloc exception, if N is less than sizeof(\-T) + (2 $\ast$ sizeof(int)) 
...
O(1) in space O(n) in time after allocation there must be enough space left for a valid block the smallest allowable block is sizeof(\-T) + (2 $\ast$ sizeof(int)) choose the first block that fits throw a bad\-\_\-alloc exception, if n is invalid 
\begin{DoxyParams}{Parameters}
{\em size\-\_\-type} & n to allocate n $\ast$ sizeof(\-T) space in heap \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
pointer to the allocated space in heap 
\end{DoxyReturn}
...
\begin{DoxyParams}{Parameters}
{\em pointer} & p, const\-\_\-reference v \\
\hline
\end{DoxyParams}
\hypertarget{classAllocator_aaf1dbab9be62941817f98618f94c7146}{\index{Allocator@{Allocator}!deallocate@{deallocate}}
...
\item[{size\-\_\-type}]{t}
\end{DoxyParamCaption}
)\hspace{0.3cm}{\ttfamily [inline]}}}\label{classAllocator_aaf1dbab9be62941817f98618f94c7146}
O(1) in space O(1) in time after deallocation adjacent free blocks must be coalesced throw an invalid\-\_\-argument exception, if p is invalid 
\begin{DoxyParams}{Parameters}
{\em pointer} & to space to deallocate in heap, size\-\_\-type t \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
none 
...
O(1) in space O(1) in time 
\begin{DoxyParams}{Parameters}
{\em int} & for array indexing \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
read-\/only reference to heap 
\end{DoxyReturn}
\hypertarget{classAllocator_ae57d6a9fc29cb1c1e61f58f90f7f3123}{\index{Allocator@{Allocator}!operator\mbox{[}$\,$\mbox{]}@{operator[]}}
\index{operator\mbox{[}$\,$\mbox{]}@{operator[]}!Allocator@{Allocator}}
...
\item[{int}]{i}
\end{DoxyParamCaption}
)\hspace{0.3cm}{\ttfamily [inline]}}}\label{classAllocator_ae57d6a9fc29cb1c1e61f58f90f7f3123}
O(1) in space O(1) in time 
\begin{DoxyParams}{Parameters}
{\em int} & for array indexing \\
\hline
